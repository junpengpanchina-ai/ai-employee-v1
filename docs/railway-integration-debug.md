...
### 9.4a 不要用 `www.worldmonitor.app` 地图页当供料 URL

[World Monitor 地图站](https://www.worldmonitor.app/)（带 `lat`、`lon`、`layers` 等参数）是浏览器里的 **HTML / 前端**，不是 JSON API。若把 `WORLDMONITOR_PUBLIC_URL=https://www.worldmonitor.app`，orchestrator 会请求 **`https://www.worldmonitor.app/api/export/intel`**，得到的通常是 **站点路由或 HTML**，无法被 `fetchWorldMonitorFeed` 解析成情报条目。

程序化数据应使用 **API 子域**（与产品文档一致），例如 **`https://api.worldmonitor.app/v1/...`**：把 **完整 REST 路径**填进 **`WORLDMONITOR_INTEL_EXPORT_URL`**，并配置 **`WORLDMONITOR_BEARER_TOKEN`**。若官方返回的 JSON 结构不是数组/`{ items }`，还需在 orchestrator 侧增加字段映射（当前实现见 `worldmonitorFeed.js` 的 `extractItemsArray`）。
