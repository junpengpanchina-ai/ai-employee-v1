...
- **定时推送老板**：二选一——(1) 外部定时器 `POST /internal/intel/push?slot=morning|noon|night`；(2) **进程内方式 A**：配齐 `TELEGRAM_BOSS_CHAT_ID`、`BOT_SERVICE_BASE_URL`、内部密钥后**可不设** `INTEL_AUTO_PUSH_ENABLED` 即自动开（默认每天 8/12/21 点，`INTEL_AUTO_PUSH_TZ` 建议 `Asia/Shanghai`）；显式 `INTEL_AUTO_PUSH_ENABLED=false` 可关。实现见 `src/intelPushScheduler.js`、`src/intelScheduledPush.js`。
